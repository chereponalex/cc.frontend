...
const DatePicker = forwardRef<HTMLInputElement, DatePickerProps>(
  (props, ref) => {
    ...
    //     const weekRange = { start: startOfWeek, end: endOfWeek };

    //     setValue(weekRange);
    //     setInputState(
    //       `${capitalize(dayjs(startOfWeek).locale(finalLocale).format(dateFormat))} - ${capitalize(dayjs(endOfWeek).locale(finalLocale).format(dateFormat))}`
    //     );
    //     closePickerOnChange && closeDropdown();
    //     window.setTimeout(() => inputRef.current?.focus(), 0);
    //   } else {

...
const DatePickerRange = forwardRef<HTMLInputElement, DatePickerRangeProps>(
  (props, ref) => {
    ...
    //   }
    // };
    const handleValueChange = (dateOrRange: Date | [Date, Date]) => {
      if (allWeekSelected) {
        const weekRange = getWeekRange(dateOrRange as Date);
        setValue(weekRange);

        if (closePickerOnChange && validationRule(weekRange)) {
          setDropdownOpened(false);
          onDropdownClose?.();
          window.setTimeout(() => inputRef.current?.focus(), 0);
        }
      } else {
        const range = dateOrRange as [Date, Date];
        setValue(range);
    ...
    return (
      ...
      >
        <RangeCalendar
          ...
          onChange={(date) => handleValueChange(date as Date | [Date, Date])}
        />
      </BasePicker>
    );
  },
